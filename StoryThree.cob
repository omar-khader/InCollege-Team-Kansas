...
display-job-detail.
    *> Open jobs file and find the selected job
    open input JobsFile

    move 0 to ws-current-job
    move "N" to WS-JOB-EOF

    perform until WS-JOB-EOF = "Y" or
                 ws-current-job = ws-job-detail-choice
        read JobsFile into JobRecord
            at end move "Y" to WS-JOB-EOF
            not at end
                add 1 to ws-current-job
                if ws-current-job = ws-job-detail-choice
                    perform parse-job-record
                end-if
        end-read
    end-perform
